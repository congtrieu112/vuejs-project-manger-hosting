<template>
  ...
  <div class="content-wrapper">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              ...
              <button class="btn btn-success btn-sm btn-add-user ml-20">Add New</button>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
    ...
    <!-- /.content -->
    <!-- MODAL EDIT USER -->
    <div class="modal fade" id="model-edit-or-add-user">
      <div class="modal-dialog">
        <div class="modal-content">
          <form id="update-service" role="form" autocomplete="off">
      ...
      columns: [
        ...
        columnDefs: [
          ...
          },
          {
            ...
            data: null,
            defaultContent:
              '<a class="btn btn-warning btn-sm btn-edit-user">Edit</a> <a class="btn btn-danger btn-sm btn-delete">Delete</a>'
          }
        ]
    });
    this.modalEl = $("#model-edit-or-add-user");
    $("body").on("click", ".btn-edit-user", async function() {
      this.modalEl = $("#model-edit-or-add-user");
      ...
      const fullname = parentsTr.find("td").eq(0).text(),
            ...
            birthday = parentsTr.find("td").eq(4).text(),
            status = parentsTr.find("td").eq(5).text();
      this.modalEl.on("shown.bs.modal", function(e) {
        $("#update-service").bootstrapValidator("resetForm", true);
      ...
      this.modalEl.on("hidden.bs.modal", function(e) {
          $("#update-service").data('bootstrapValidator').resetForm();
      });
      $("#update-service").bootstrapValidator({
         live: 'enabled',
        fields: {
          email: {
            validators: {
        ...
        onSuccess:async function(e, data) {
          ...
          return false;
            ...
            if ($(this).val().length == 1) {
                $("#update-service").bootstrapValidator('validateField', 'password')
                                .bootstrapValidator('validateField', 'repassword');
            }
        });

    });
    $("body").on("click", ".btn-add-user", async function() {
      this.modalEl = $("#model-edit-or-add-user");
      ...
      this.modalEl.find("input[name=birthday]").datepicker({ autoclose: true,format: 'yyyy-mm-dd'}).on('changeDate', function(e) {
            ...
            $("#update-service").bootstrapValidator('revalidateField', 'birthday');
      });
